...
import me.justup.upme.db_upme.providers.ProductsCategoriesProvider;

import static me.justup.upme.utils.LogUtils.LOGD;
import static me.justup.upme.utils.LogUtils.LOGE;
import static me.justup.upme.utils.LogUtils.LOGI;

public class ProductCategoriesSyncAdapterService extends Service implements SyncAdapterMetaData, RequestServiceCallbackListener {
    private static final String TAG = ProductCategoriesSyncAdapterService.class.getSimpleName();
    ...
    public ProductCategoriesSyncAdapterService() {
        super();
    }
    ...
    private static class SyncAdapterImplementation extends AbstractThreadedSyncAdapter {
        ...
        public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
            try {
                ProductCategoriesSyncAdapterService.performSync(mContext, account, extras, authority, provider, syncResult);
    ...
    private static void performSync(Context context, Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) throws OperationCanceledException {
        LOGD(TAG, "ContentProviderClient : " + provider.toString() + ", Local Content Provider : " + provider.getLocalContentProvider().getClass().getName());

        if (provider.getLocalContentProvider() instanceof ProductsCategoriesProvider) {
            ...
            if (checkConnectingToInternet(context)) {
                ...
                startIntent.putExtra(Constants.SYNC_STATUS_TEXT, R.string.sync_adapter_product_categories_sync);
                ...
                if(mJustUpApplication!=null) {
                    if (mJustUpApplication.getTransferActionBrandCategories() == null) {
                        mJustUpApplication.setTransferActionBrandCategories(account.name);
                    }
    ...
    public void onServiceCallback(int requestId, Intent requestIntent, int resultCode, Bundle data) {
        ...
        switch (action){
            case me.justup.upme.api_rpc.utils.Constants.ACTION_PRODUCTS_GET_ALL_CATEGORIES :
                if(resultCode== AppIntentHandler.SUCCESS_RESPONSE){
                    ProductsObject productsObject = (ProductsObject) data.getSerializable(AppIntentHandler.EXTRA_PARAM_MAIL_RESPONSE);
                    LOGD(TAG, "ProductsObject : " + productsObject.toString());

                    JustUpApplication.getApplication().getTransferActionBrandCategories().insertListBrandCategories(getApplicationContext(), productsObject.getBrandCategories());
                }else{
        ...
        intentEnd.putExtra(Constants.SYNC_STATUS_TEXT, R.string.sync_adapter_product_categories_sync);
