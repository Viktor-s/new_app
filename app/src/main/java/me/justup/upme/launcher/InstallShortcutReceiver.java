...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    private static void processInstallShortcut(Context context, PendingInstallShortcutInfo pendingInfo) {
        ...
        synchronized (app) {
            ...
            final int screen = LauncherActivity.DEFAULT_SCREEN;
            for (int i = 0; i < (2 * LauncherActivity.SCREEN_COUNT) + 1 && !found; ++i) {
                int si = screen + (int) ((i / 2f) + 0.5f) * ((i % 2 == 1) ? 1 : -1);
                if (0 <= si && si < LauncherActivity.SCREEN_COUNT) {
    ...
    private static boolean installShortcut(Context context, Intent data, ArrayList<ItemInfo> items,
        ...
        if (findEmptyCell(context, items, tmpCoordinates, screen)) {
            if (intent != null) {
                ...
                boolean duplicate = data.getBooleanExtra(LauncherActivity.EXTRA_SHORTCUT_DUPLICATE, true);
